...
def writeImageData(img):
    name = "./result/result.jpg"
    if os.path.exists(name):
        os.remove(name)
    cv2.imwrite(name, img)
    
def keep_letters(text):
    pattern = r'[a-zA-Z]+'  # 正則表達式模式，匹配一個或多個字母
...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    global recLeft, recRight, pokemon
    ...
    current_file_path = "./yolov7/utils/plot.py"
    current_dir = os.path.dirname(current_file_path)
    
    textPath = "../result/imagedata.txt"
    other_folder_path = os.path.join(os.path.dirname(current_dir), textPath)
    imgFile = open(other_folder_path, "a")
    content = keep_letters(label) + '\n'
